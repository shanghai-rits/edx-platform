...
def validate_user_displayname(user_displayname):
    """
    Verifies a username_displayname is valid, raises a ValidationError otherwise.
    Args:
        username_displayname (unicode): The username_displayname to validate.

    Unicode allowed by default.
    """

    user_displayname_re = fr"^{settings.USERNAME_REGEX_PARTIAL}$"
    flags = re.UNICODE
    message = accounts.USER_DISPLAYNAME_INVALID_CHARS_UNICODE

    validator = RegexValidator(
        regex=user_displayname_re,
        flags=flags,
        message=message,
        code='invalid',
    )

    validator(user_displayname)

def validate_phone(phone):
    """
    Validate phone number
    """
    phone_re = r"^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\d{8}$"
    flags = re.UNICODE
    message = accounts.PHONE_INVALID_CHARS_UNICODE
...
def validate_sms_code(sms_code):
    """
    Validate sms code
    """
    sms_code_re = r"^\d{6}$"
    flags = re.UNICODE
    message = accounts.SMS_CODE_INVALID_CHARS_UNICODE
...
class PhoneAccountCreationForm(forms.Form):
    """
    ...
    _PHONE_INVALID_MSG = _("A properly formatted phone number is required")
    _USER_DISPLAYNAME_TOO_SHORT_MSG = _("Your legal user_displayname must be a minimum of two characters long")
    ...
    phone = forms.CharField(
        min_length=1,
        error_messages={
            "required": _PHONE_INVALID_MSG,
            "min_length": _PHONE_INVALID_MSG,
        }
    )

    user_displayname = forms.CharField(
        min_length=1,
        error_messages={
            "required": _USER_DISPLAYNAME_TOO_SHORT_MSG,
            "min_length": _USER_DISPLAYNAME_TOO_SHORT_MSG,
        }
    )
    ...
    def clean_phone(self):
        """ Enforce phone restrictions (if applicable) """
    ...
    def clean_sms_code(self):
        """ Enforce sms code restrictions (if applicable) """
        sms_code = self.cleaned_data["sms_code"]
        if not(re.match("^[0-9]{6}$", sms_code)):
            raise ValidationError(_("Invalid sms code."))
        return sms_code
...
class PhoneRegistrationFormFactory:
    """
    Construct Registration forms and associated fields.
    """

    DEFAULT_FIELDS = ["user_displayname", "phone", "sms_code"]
    ...
    def _add_user_displayname_field(self, form_desc, required=True):
        ...
        form_desc.add_field(
            ...
            restrictions={
                "min_length": accounts.USER_DISPLAYNAME_MIN_LENGTH,
                "max_length": accounts.USER_DISPLAYNAME_MAX_LENGTH,
            },
            required=required
        )

...
from openedx.core.djangoapps.user_api.accounts.api import (
    ...
    get_user_displayname_validation_error,
    get_phone_validation_error,
    get_sms_code_validation_error,
    get_username
)
...
from openedx.core.djangoapps.user_authn.views.phone_registration_form import (
    PhoneAccountCreationForm,
    PhoneRegistrationFormFactory
)
...
from common.djangoapps.track import segment, rudderstack
from common.djangoapps.util.db import outer_atomic
from common.djangoapps.util.json_request import JsonResponse

from edx_django_utils.user import generate_password

from common.djangoapps.third_party_auth.sms import check_phone_exists
...
def create_account_with_params(request, params):
    ...
    * Duplicate email raises a ValidationError (rather than the expected
    ...
    if is_third_party_auth_enabled and (pipeline.running(request) or third_party_auth_credentials_in_api) or params.get('sms_code', ''):
        params["password"] = generate_password()
    
    # Third party login email
    if is_third_party_auth_enabled and (pipeline.running(request) or third_party_auth_credentials_in_api) and not params.get('email', ''):
        username = params['username']
        params['email'] = '{username}@example.com'.format(username=username)
    ...
    if params.get('sms_code', ''):
        # create user with sms code
        # validations
        # check sms code
        username = params['username']
        phone = params['phone']
        sms_code = params['sms_code']
        conflicts = third_party_auth.sms.sms_code_verification(phone, sms_code, 'register', request.session)
        if conflicts:
    ...
    phone_form = PhoneAccountCreationForm(
            data=params,
    ...
    with outer_atomic():
        ...
        if params.get('sms_code', ''):
            (user, profile, registration) = do_create_account(phone_form, custom_form)
            try:
                phone = params['phone']
                phone = third_party_auth.phone.Phone(phone)
                UserSocialAuth.create_social_auth(user=user, uid=phone.make_phone(), provider='sms').set_extra_data(
                    {'phone': phone.fuzzy()})
            except Exception:  # pylint: disable=broad-except
                log.exception("UserSocialAuth creation failed for user {id}.".format(id=user.id))
                raise
            ...
            new_user = authenticate_new_user(request, user.username, phone_form.cleaned_data['password'])
        else:
    ...
    skip_email = _skip_activation_email(
        user, running_pipeline, params, third_party_provider,
    )
    ...
    if hasattr(settings, 'LMS_SEGMENT_KEY') and settings.LMS_SEGMENT_KEY:
        identity_args = [
            user.id,
            {
                ...
                'username': user.username,
                'name': profile.name,
                'user_displayname': UserExtension.get_displayname(user),
        ...
        rudderstack.identify(*identity_args)
        properties = {
            'category': 'conversion',
...
def _skip_activation_email(user, running_pipeline, params, third_party_provider):
    """
...
class RegistrationView(APIView):
    ...
    def post(self, request):
        ...
        # The user display name replaces the original name 
        data['username'] = get_username()
        data['name'] = data['user_displayname']
        self._handle_terms_of_service(data)

        # Mobile applications do not send phone field but they send the sms_code field
        # TODO: Mobile application - Send phone field in the mobile application
        if (data.get('phone') is None) and data.get('sms_code'):
...
class RegistrationValidationView(APIView):
    ...
    def user_displayname_handler(self, request):
        """ Validates whether the user_displayname is valid. """
        user_displayname = request.data.get('user_displayname')
        invalid_user_displayname_error = get_user_displayname_validation_error(user_displayname)
        user_displayname_exists_error = UserExtension.check_username_exists(user_displayname)
        # We prefer seeing for invalidity first.
        # Some invalid user_displaynames (like for superusers) may exist.
        return invalid_user_displayname_error or user_displayname_exists_error
    
    def phone_handler(self, request):
        """ Validates whether the phone is valid. """
        phone = request.data.get('phone')
        invalid_phone_error = get_phone_validation_error(phone)
        phone_exists = check_phone_exists(phone)
        phone_exists_error = ''
        if phone_exists:
            phone_exists_error = _(
                    "It looks like {phone_number} belongs to an existing account. Try again with "
                    "a different phone number."
                ).format(phone_number=phone)
        # We prefer seeing for invalidity first.
        # Some invalid phones (like for superusers) may exist.
        # return invalid_phone_error or phone_exists_error
        return invalid_phone_error or phone_exists_error

    def sms_code_handler(self, request):
        """ Validates whether the sms code is valid. """
        sms_code = request.data.get('sms_code')
        invalid_sms_code_error = get_sms_code_validation_error(sms_code)

        sms_code_requested = (request.session.get('expires') is None) and (request.data.get('password') is None) # Handle mobile applications
        sms_code_requested_error = ""
        if sms_code_requested:
            sms_code_requested_error = _(
                "Please request a new code."
            )
        # We prefer seeing for invalidity first.
        # Some invalid phones (like for superusers) may exist.
        # return invalid_phone_error or phone_exists_error
        return invalid_sms_code_error or sms_code_requested_error
    ...
    validation_handlers = {
        ...
        "user_displayname": user_displayname_handler,
        "email": email_handler,
        "confirm_email": confirm_email_handler,
        "password": password_handler,
        "country": country_handler,
        "phone": phone_handler,
        "sms_code": sms_code_handler,
    }

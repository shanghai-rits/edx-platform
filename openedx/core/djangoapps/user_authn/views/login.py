...
from openedx.core.djangoapps.user_api.views import require_post_params
...
def _get_user_by_email(request):
    ...
    try:
        ...
        try: 
            phone = third_party_auth.phone.Phone(email)
            user_sms_auth = UserSocialAuth.get_social_auth('sms', phone.make_phone())
            if not user_sms_auth:
                raise UserSocialAuth.DoesNotExist()
            user = user_sms_auth.user
        except UserSocialAuth.DoesNotExist:
            digest = hashlib.shake_128(email.encode('utf-8')).hexdigest(16)  # pylint: disable=too-many-function-args
            AUDIT_LOG.warning(f"Login failed - Unknown user email or phone number: {digest}")

def _get_user_by_phone(request):
    """
    Finds a user object in the database based on the given request, ignores all fields except for phone.
    """
    if 'phone' not in request.POST or 'sms_code' not in request.POST:
        raise AuthFailedError(_('There was an error receiving your login information. Please email us.'))
    ...
    try: 
        phone = third_party_auth.phone.Phone(phone_number)
        user_sms_auth = UserSocialAuth.get_social_auth('sms', phone.make_phone())
        if not user_sms_auth:
            raise UserSocialAuth.DoesNotExist()
        user = user_sms_auth.user
        user.phone = request.POST['phone']
        user.sms_code = request.POST['sms_code']
        return user
    except UserSocialAuth.DoesNotExist:
        digest = hashlib.shake_128(phone_number.encode('utf-8')).hexdigest(16)  # pylint: disable=too-many-function-args
        AUDIT_LOG.warning(f"Login failed - Unknown user phone number: {digest}")
        raise AuthFailedError(
                _(
                    u"Unknown phone number."
                )
            )
...
def _authenticate_phone(request, unauthenticated_user, third_party_auth_requested):
    """
    Use Django authentication on the given request, using rate limiting if configured
    """
    should_be_rate_limited = getattr(request, 'limited', False)
    ...
    # If the user doesn't exist, we want to set the username to an invalid username so that authentication is guaranteed
    # to fail and we can take advantage of the ratelimited backend
    phone = unauthenticated_user.phone if unauthenticated_user else ""
    sms_code = unauthenticated_user.sms_code if unauthenticated_user else ""

    # Verify SMS code
    try:
        conflicts = third_party_auth.sms.sms_code_verification(phone, sms_code, 'login', request.session)
        if conflicts:
            raise AuthFailedError(
                _(
    ...
    authenticate = third_party_auth.backends.SmsCodeBackend.authenticate
    ...
    possibly_authenticated_user.backend = "third_party_auth.backends.SmsCodeBackend"
    
    return possibly_authenticated_user
...
def login_user(request, api_version='v1'):
    ...
    phone_auth_requested = bool(request.POST.get('phone'))
    is_user_third_party_authenticated = False
    is_user_phone_authenticated = False

    set_custom_attribute('login_user_course_id', request.POST.get('course_id'))

    if is_require_third_party_auth_enabled() and not third_party_auth_requested and not phone_auth_requested:
    ...
    try:
        if third_party_auth_requested and not first_party_auth_requested and not phone_auth_requested:
        ...
        elif phone_auth_requested:
            user = _get_user_by_phone(request)
            is_user_phone_authenticated = True
        else:
        ...
        if not is_user_third_party_authenticated and not is_user_phone_authenticated:
        ...
        if is_user_phone_authenticated:
            possibly_authenticated_user = _authenticate_phone(request, user, third_party_auth_requested) 
...
def reset_password(request):
    if request.method == 'GET':
        return render_to_response('reset_password.html', {'disable_footer': True})
    """Handle password change requests originating from the login page.

        Uses the Account API to email the user a link to the password reset page.

        Note:
            If use email to reset password:
                Forward to function: password_change_request_handler
            If use phone to reset password:
                Redirect to the link of password reset page.
        ...
        Returns:
            ...
            HttpResponse: 400 if there is no 'email' or 'phone', 'sms_code', 'action' POST parameter
    ...
    if 'email' in request.POST:
        ...
        return password_change_by_sms_code(request)
    else:
        return HttpResponseBadRequest('Missing parameter: "email" or "phone".')


@require_post_params(['phone', 'sms_code'])
def password_change_by_sms_code(request):
    phone = request.POST.get('phone')
    sms_code = request.POST.get('sms_code')
    try:
        authenticate = third_party_auth.backends.SmsCodeBackend.authenticate
        user = authenticate(request, phone=phone, action="reset_password", sms_code=sms_code)
    except third_party_auth.backends.SmsCodeError as e:
        return HttpResponse(json.dumps(e.user_message), status=401)
    if user is None:
        return HttpResponse(json.dumps({
            "success": False,
            "value": _("No such user for phone: {}").format(phone)
        }), status=401)

    def reset_pwd_address(user):
        ...
        url = reverse('password_reset_confirm', kwargs={'uidb36': uid, 'token': token})
        return '{protocol}://{site_name}{url}'.format(protocol=protocol, site_name=site_name, url=url)

    address = reset_pwd_address(user)
    return HttpResponse(json.dumps({
        "success": True,
        "value": address
    }))
...
def request_post_phone(group, request) -> str:  # pylint: disable=unused-argument
    """
    Return the the phone post param if it exists, otherwise return a
    random id.

    If the request doesn't have an phone post body param, treat it as
    a unique key. This will probably mean that it will not get rate limited.
    """

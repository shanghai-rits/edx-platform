...
from openedx.core.djangoapps.user_authn.views.registration_form import validate_name, validate_username, validate_user_displayname
from openedx.core.djangoapps.user_authn.views.phone_registration_form import validate_phone, validate_sms_code
from openedx.core.lib.api.view_utils import add_serializer_errors
from openedx.features.enterprise_support.utils import get_enterprise_readonly_account_fields
from .serializers import AccountLegacyProfileSerializer, AccountUserSerializer, AccountUserExtensionSerializer, UserReadOnlySerializer, _visible_fields
...
def update_account_settings(requesting_user, update, username=None):
    ...
    # If user has change user displayname
    if 'user_displayname'in update:
        update['displayname'] = update['user_displayname']
        update.pop('user_displayname')
    ...
    extension_serializer = AccountUserExtensionSerializer(user.extension, data=update)
    for serializer in user_serializer, legacy_profile_serializer, extension_serializer:
...
def get_user_displayname_validation_error(user_displayname):
    """Get the built-in validation error message for when
    the user displayname is invalid in some way.

    :param user_displayname: The proposed user displayname (unicode).
    :param default: The message to default to in case of no error.
    :return: Validation error message.

    """
    return _validate(_validate_user_displayname, errors.AccountUserDisplaynameInvalid, user_displayname)

def get_phone_validation_error(phone):
    """Get the built-in validation error message for when
    the phone is invalid in some way.

    :param phone: The proposed phone (unicode).
    :param default: The message to default to in case of no error.
    :return: Validation error message.
...
def get_sms_code_validation_error(sms_code):
    """Get the built-in validation error message for when
    the sms code is invalid in some way.

    :param sms_code: The proposed sms code (unicode).
    :param default: The message to default to in case of no error.
    :return: Validation error message.

    """
    return _validate(_validate_sms_code, errors.AccountSmsCodeInvalid, sms_code)
...
def _validate_user_displayname(user_displayname):
    """Validate the user_displayname.

    Arguments:
        user_displayname (unicode): The proposed user_displayname.

    Returns:
        None

    Raises:
        errors.AccountUserDisplaynameInvalid

    """
    try:
        _validate_unicode(user_displayname)
        _validate_type(user_displayname, str, accounts.USER_DISPLAYNAME_BAD_TYPE_MSG)
        _validate_length(
            user_displayname,
            accounts.USER_DISPLAYNAME_MIN_LENGTH,
            accounts.USER_DISPLAYNAME_MAX_LENGTH,
            accounts.USER_DISPLAYNAME_BAD_LENGTH_MSG
        )
        with override_language('en'):
            # `validate_user_displayname` provides a proper localized message, however the API needs only the English
            # message by convention.
            validate_user_displayname(user_displayname)
    except (UnicodeError, errors.AccountDataBadType, errors.AccountDataBadLength) as user_displayname_err:
        raise errors.AccountUserDisplaynameInvalid(str(user_displayname_err))
    except ValidationError as validation_err:
        raise errors.AccountUserDisplaynameInvalid(validation_err.message)

def _validate_phone(phone, api_version='v1'):
    """Validate the phone.

    Arguments:
        phone (unicode): The proposed phone.

    Returns:
        None

    Raises:
        errors.AccountPhoneInvalid

    """
    try:
        _validate_unicode(phone)
        _validate_type(phone, str, accounts.PHONE_BAD_TYPE_MSG)
        _validate_length(
            phone,
            accounts.PHONE_LENGTH,
            accounts.PHONE_LENGTH,
            accounts.PHONE_BAD_LENGTH_MSG
        )
        with override_language('en'):
            # `validate_phone` provides a proper localized message, however the API needs only the English
            # message by convention.
            validate_phone(phone)
    except (UnicodeError, errors.AccountDataBadType, errors.AccountDataBadLength) as phone_err:
        raise errors.AccountPhoneInvalid(str(phone_err))
    except ValidationError as validation_err:
        raise errors.AccountPhoneInvalid(validation_err.message)

def _validate_sms_code(sms_code, api_version='v1'):
    """Validate the sms_code.

    Arguments:
        sms_code (unicode): The proposed sms_code.

    Returns:
        None

    Raises:
        errors.AccountSmsCodeInvalid

    """
    try:
        _validate_unicode(sms_code)
        _validate_type(sms_code, str, accounts.SMS_CODE_BAD_TYPE_MSG)
        _validate_length(
            sms_code,
            accounts.SMS_CODE_LENGTH,
            accounts.SMS_CODE_LENGTH,
            accounts.SMS_CODE_BAD_LENGTH_MSG
        )
        with override_language('en'):
            # `validate_sms_code` provides a proper localized message, however the API needs only the English
            # message by convention.
            validate_sms_code(sms_code)
    except (UnicodeError, errors.AccountDataBadType, errors.AccountDataBadLength) as sms_code_err:
        raise errors.AccountSmsCodeInvalid(str(sms_code_err))
    except ValidationError as validation_err:
        raise errors.AccountSmsCodeInvalid(validation_err.message)
...
def get_username():
    time.time()
    last_id = User.objects.filter().aggregate(max_id=Max('pk')).get('max_id') + 1
    timestamp = int(time.time())
    return str(timestamp + last_id)
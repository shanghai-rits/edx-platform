...
from .accounts import (
    ...
    USERNAME_MIN_LENGTH, USERNAME_MAX_LENGTH, USER_DISPLAYNAME_MIN_LENGTH, USER_DISPLAYNAME_MAX_LENGTH
)
from .accounts.api import check_account_exists, get_username
from common.djangoapps.util.enterprise_helpers import insert_enterprise_fields
...
from openedx.core.djangoapps.user_api.accounts.api import get_username
from social_django.models import UserSocialAuth
from openedx.core.lib.api.permissions import ApiKeyHeaderPermission

def check_user_is_teacher(user):
    if user.is_staff:
        return True
    else:
        try:
...
class LoginSessionView(APIView):
    ...
    def get(self, request):
        ...
        # email_label = _(u"Email or Phone")
        email_label = _(u"Email")
        
        # Translators: This example email address is used as a placeholder in
        # a field on the login form meant to hold the user's email address.
        # email_placeholder = _(u"You email address or phone number")
        email_placeholder = _(u"Your email address")
...
class RegistrationView(APIView):
    ...
    DEFAULT_FIELDS = ["email", "user_displayname", "password"]
    ...
    def post(self, request):
        ...
        # The user has already authenticated via third-party auth
        should_link_with_social_auth = third_party_auth.is_enabled() and 'provider' in dict(data.items())

        if should_link_with_social_auth or (third_party_auth.is_enabled() and third_party_auth.pipeline.running(request)):
            data['email'] = '{}@default.com'.format(username)
        ...
        if sms_code:
            conflicts = third_party_auth.sms.sms_code_verification(email, sms_code, 'register', request.session)
        if conflicts:
            conflict_messages = {
                "sms_code": _(
        ...
        # if user existed and sms_code has verified
        phone_existed = third_party_auth.sms.check_phone_exists(email)
        if sms_code and phone_existed:
            # user = UserSocialAuth.get_social_auth(provider='sms', uid=third_party_auth.phone.Phone(email).make_phone()).user
            # response = JsonResponse({"success": True})
            # from django.contrib.auth import login, get_backends
            # # user.backend = get_backends()
            # login(request, user, "third_party_auth.backends.SmsCodeBackend")
            # request.session.set_expiry(0)
            # set_logged_in_cookies(request, response, user)
            # return response
            conflicts.append("phoneNum")
            conflicts.append("username")            
            conflict_messages = {
                "phoneNum": _(
                    # Translators: This message is shown to users who attempt to create a new
                    # account using an email address associated with an existing account.
                    u"It looks like {phone_number} belongs to an existing account. "
                    u"Try again with different phone number."
            ...
            return JsonResponse(errors, status=409)
        elif sms_code and not phone_existed:
            data['phone'] = email
            data['email'] = '{}@default.com'.format(username)
    ...
    def _add_user_displayname_field(self, form_desc, required=True):
        user_displayname_label = _(u"Nickname")

        user_displayname_instructions = _(
            u""
        ).format(bold_start=u'<strong>', bold_end=u'</strong>')

        user_displayname_placeholder = _(u"JaneDoe")

        form_desc.add_field(
            "user_displayname",
            label=user_displayname_label,
            instructions=user_displayname_instructions,
            placeholder=user_displayname_placeholder,
            restrictions={
                "min_length": USER_DISPLAYNAME_MIN_LENGTH,
                "max_length": USER_DISPLAYNAME_MAX_LENGTH,
            },
            required=required
        )
    ...
    def _add_sms_code_field(self, form_desc, required=True):
        ...
        sms_code_instructions = _('').format(bold_start=u'<strong>', bold_end=u'</strong>')
    ...
    def _add_level_of_education_field(self, form_desc, required=True):
        ...
        options = [(name, _(label)) for name, label in
                   UserProfile.LEVEL_OF_EDUCATION_CHOICES]  # pylint: disable=translation-of-non-string
        form_desc.add_field(
            "level_of_education",
    ...
    def _add_gender_field(self, form_desc, required=True):
        ...
        # The labels are marked for translation in UserProfile model definition.
        options = [(name, _(label)) for name, label in
                   UserProfile.GENDER_CHOICES]  # pylint: disable=translation-of-non-string
        form_desc.add_field(
            "gender",
            label=gender_label,
            field_type="select",
    ...
    def _add_country_field(self, form_desc, required=True):
        ...
        error_msg = _(u"Please select your Country or Region.")

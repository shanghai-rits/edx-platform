...
class EdxOAuth2Validator(OAuth2Validator):
    ...
    def validate_user(self, username, password, client, request, *args, **kwargs):
        ...
        to authenticate.
        """
        user = self._authenticate(username=username, password=password, request=request)
        if user is not None:
            request.user = user
            return True
        return False

    def _authenticate(self, username, password, request):
        """
        Authenticate the user, allowing the user to identify themselves either
        by username or email
        """

        sms_code_request = request._params.get('sms_code') is not None

        authenticated_user = authenticate(username=username, password=password)
        if authenticated_user is None and not sms_code_request:
        ...
        elif sms_code_request:
            phone_number = request._params.get('phone')
            if phone_number:
                authenticated_user = UserSocialAuth.get_social_auth('sms', Phone(phone_number).make_phone()).user

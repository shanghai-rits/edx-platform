...
from common.djangoapps.third_party_auth.phone import Phone
...
class AccessTokenView(_DispatchingView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        grant_type_password = False

        if request.POST.get('grant_type') == 'password':
            grant_type_password = True
        
        if grant_type_password:
            # log.info("Mobile Application Registration Flow")
            ...
            phone_user_exists = UserSocialAuth.get_social_auth(provider='sms', uid=Phone(username).make_phone())

            user_complete_registration = False

            if user_exists:
                user = User.objects.get(email=username)
            elif phone_user_exists:
                user = UserSocialAuth.get_social_auth(provider='sms', uid=Phone(username).make_phone()).user

            if user_exists or phone_user_exists:
                user_date_joined = user.date_joined.replace(second=0, microsecond=0)
                user_last_login = user.last_login.replace(second=0, microsecond=0)

                # log.info(user_date_joined)
                # log.info(user_last_login)

                if user_date_joined == user_last_login:
                    user_complete_registration = True
            
            if not user_complete_registration and user_exists:
                # log.info("User exists")
                is_user_active = User.objects.get(email=username).is_active
            ...
            # Normal Phone Login flow
            if request.POST.get('password') is None or request.POST.get('phone'):
                ...
                phone_number_user_exists = UserSocialAuth.get_social_auth(provider='sms', uid=Phone(phone_number).make_phone())
                if not phone_number_user_exists:
                    return JsonResponse({
                        "phone": _(
                ...
                if phone_number and sms_code:
                    ...
                    conflicts = third_party_auth.sms.sms_code_verification(phone_number, sms_code, 'login', request.session)
                    ...
                    if conflicts:
                        ...
                        return JsonResponse(errors, status=410)


                post['phone'] = phone_number    
                post['password'] = 'filler'
                request.POST = post

            # Newly registered phone user
            elif phone_user_exists and user_complete_registration and request.POST.get('password') == settings.MOBILE_APPLICATION_SECRET:
                # log.info("Login after register with phone")
                post = request.POST.copy()
                post['phone'] = username
                post['sms_code'] = '133400'
                request.POST = post
                # log.info(post)
                request.session['action'] = 'login'
            
            # Handle other requests that will login
            elif request.POST.get('sms_code'):
                return JsonResponse({
                    "sms_code": _(
                        u"Incorrect verification sms code."
                    ),

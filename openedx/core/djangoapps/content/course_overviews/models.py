...
from common.djangoapps.facet.models import Facet, FacetOfCourse
from django.conf import settings
...
class CourseOverview(TimeStampedModel):
    ...
    @property
    def courses_facet(self):
        try:
            number = Facet.objects.get(displayname=configuration_helpers.get_value('HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME', settings.HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME)).number
            group = FacetOfCourse.objects.get(facet=number, course=self.id).group
        except FacetOfCourse.DoesNotExist as e:
            log.warning(e)
            group = None
        return group
...
class CourseOverviewExpansion(models.Model):
    course_overview = models.CharField(max_length=255)
    chinese_display_name = models.TextField(null=True)
    eng_display_name = models.TextField(null=True, blank=True, default='')
    vertical_cover_path = models.TextField(blank=True, default="")

    @classmethod
    def get_value_by_name(cls, name, course_key):
        try:
            value = getattr(cls.objects.get(course_overview=course_key), name, '')
    ...
    @classmethod
    def get_chinese_display_name(cls, course_key):
        return cls.get_value_by_name('chinese_display_name', course_key)

    @classmethod
    def get_eng_display_name(cls, course_key):
        return cls.get_value_by_name('eng_display_name', course_key)

    @classmethod
    def get_vertical_cover_path(cls, course_key):
        return cls.get_value_by_name('vertical_cover_path', course_key)

...
from common.djangoapps.recommendation.models import RecommendedCourses
from common.djangoapps.facet.models import Facet
from common.djangoapps.facet.models import FacetOfCourse
...
class CourseDetails:
    ...
    def __init__(self, org, course_id, run):
        ...
        self.overviews = {}  # dic of texts to be used in overview sections
        self.course_about_html_editor = None
        ...
        self.course_vertical_image_asset_path = ""  # URL of the course image
    ...
    @classmethod
    def lazy_load(cls):
        from openedx.core.djangoapps.content.course_overviews.models import CourseOverviewExpansion, CourseOverview
        return CourseOverviewExpansion, CourseOverview
    ...
    def populate(cls, course_descriptor):
        ...
        course_details.course_about_html_editor = course_descriptor.course_about_html_editor
        course_details.recommendation_enabled = 'true' if (RecommendedCourses.objects.count() < 4) or (RecommendedCourses.objects.filter(unicodecoursekey=course_key).exists()) else 'limit'
        course_details.recommended = 'true' if (RecommendedCourses.objects.filter(unicodecoursekey=course_key).exists()) else 'false'

        for facet in Facet.objects.all():
            course_details.facets[facet.number] = ''
        CourseOverviewExpansion, CourseOverview = cls.lazy_load()
        try:
            course_details.course_vertical_image_asset_path = CourseOverviewExpansion.objects.get(course_overview=course_key).vertical_cover_path
        except CourseOverviewExpansion.DoesNotExist as e:
            logging.warn("Course with key {course_key} does not have vertical banner image.")
            course_details.course_vertical_image_asset_path = ""

        for record in FacetOfCourse.objects.filter(course=course_key):
            course_details.facets[record.facet] = record.group
        ...
        for attribute in ABOUT_ATTRIBUTES:
            ...
            if attribute == 'overview' and course_details.course_about_html_editor is False:
                try:
                    soup = BeautifulSoup(value, "html.parser")
                    course_details.overview = soup.prettify()
                    course_details.overviews['about'] = ''
                    course_details.overviews['prerequisites'] = ''
                    course_details.overviews['staff'] = []
                    course_details.overviews['faq'] = []
                    about = soup.find('section', class_='about')
                    if about is None:
                        course_details.overviews['include_course_about'] = False
                    else:
                        course_details.overviews['include_course_about'] = True
                        # remove title
                        about.find('h2').decompose()
                        about.find('hr').decompose()
                        course_details.overviews['about'] = about.get_text().strip()
                        
                    prerequisites = soup.find('section', class_='prerequisites')
                    if prerequisites is None:
                        course_details.overviews['include_course_prerequisites'] = False
                    else:
                        course_details.overviews['include_course_prerequisites'] = True
                        prerequisites.find('h2').decompose()
                        prerequisites.find('hr').decompose()
                        course_details.overviews['prerequisites'] = prerequisites.get_text().strip()

                    staff = soup.find('section', class_='course-staff')
                    if staff is None:
                        course_details.overviews['include_course_staff'] = False
                    else:
                        course_details.overviews['include_course_staff'] = True
                        course_details.overviews['staff'] = [
                            {'img': a.img['src'].strip(), 'h3': a.h3.get_text().strip(), 'p': a.p.get_text().strip(), 'link': a.a['href']} for a in
                            staff.find_all('article')]

                    faq = soup.find('section', class_='faq')
                    if faq is None:
                        course_details.overviews['include_course_faq'] = False
                    else:
                        course_details.overviews['include_course_faq'] = True
                        course_details.overviews['faq'] = [{'h3': a.h3.get_text().strip(), 'p': a.p.get_text().strip()} for a in
                                                           faq.find_all('article')]

                except (KeyError, AttributeError):
                    course_details.course_about_html_editor = True
    ...
    def update_from_json(cls, course_key, jsondict, user):  # pylint: disable=too-many-statements
        ...
        if 'course_vertical_image_asset_path' in jsondict:
            CourseOverviewExpansion, CourseOverview = cls.lazy_load()
            course_overviews_expansion = CourseOverviewExpansion.objects.get(course_overview=descriptor.id)
            if jsondict['course_vertical_image_asset_path'] != course_overviews_expansion.vertical_cover_path:
                course_overviews_expansion.vertical_cover_path = jsondict['course_vertical_image_asset_path']
                course_overviews_expansion.save()
        ...
        if 'course_about_html_editor' in jsondict:
            descriptor.course_about_html_editor = jsondict['course_about_html_editor']
            dirty = True
            if jsondict['course_about_html_editor'] is False and 'overviews' in jsondict:
                soup = BeautifulSoup("<html><body></body></html>", 'html.parser')
                root_tag = soup.body
                if jsondict['overviews']['include_course_about']:
                    section_tag = soup.new_tag("section")
                    section_tag['class'] = 'about'
                    h2_tag = soup.new_tag("h2")
                    h2_tag.string = '课程概述 / About This Course'
                    section_tag.append(h2_tag)
                    hr_tag = soup.new_tag("hr")
                    section_tag.append(hr_tag)
                    for p in jsondict['overviews']['about'].split('\n'):
                        p_tag = soup.new_tag("p")
                        p_tag.string = p
                        section_tag.append(p_tag)
                    root_tag.append(section_tag)

                if jsondict['overviews']['include_course_prerequisites']:
                    section_tag = soup.new_tag("section")
                    section_tag['class'] = 'prerequisites'
                    h2_tag = soup.new_tag("h2")
                    h2_tag.string = '先修要求 / Requirements'
                    section_tag.append(h2_tag)
                    hr_tag = soup.new_tag("hr")
                    section_tag.append(hr_tag)
                    for p in jsondict['overviews']['prerequisites'].split('\n'):
                        p_tag = soup.new_tag("p")
                        p_tag.string = p
                        section_tag.append(p_tag)
                    root_tag.append(section_tag)

                if jsondict['overviews']['include_course_staff']:
                    section_tag = soup.new_tag("section")
                    section_tag['class'] = 'course-staff'
                    h2_tag = soup.new_tag("h2")
                    h2_tag.string = '教师 / Faculty'
                    section_tag.append(h2_tag)
                    hr_tag = soup.new_tag("hr")
                    section_tag.append(hr_tag)
                    for a in jsondict['overviews']['staff']:
                        if a is not None:
                            article_tag = soup.new_tag("article")
                            article_tag['class'] = 'teacher'
                            div_tag = soup.new_tag("div")
                            div_tag['class'] = 'teacher-image'
                            img_tag = soup.new_tag("img", src=a['img'].strip(), align="left", style="margin:0 20 px 0")
                            div_tag.append(img_tag)
                            h3_tag = soup.new_tag("h3")
                            a_tag = soup.new_tag("a", href=a['link'])
                            a_tag.append(a['h3'])
                            h3_tag.append(a_tag)
                            p_tag = soup.new_tag("p")
                            p_tag.string = a['p']
                            article_tag.append(div_tag)
                            article_tag.append(h3_tag)
                            article_tag.append(p_tag)
                            section_tag.append(article_tag)
                    root_tag.append(section_tag)

                if jsondict['overviews']['include_course_faq']:
                    section_tag = soup.new_tag("section")
                    section_tag['class'] = 'faq'
                    inner_section_tag = soup.new_tag("section")
                    inner_section_tag['class'] = 'responses'
                    h2_tag = soup.new_tag("h2")
                    h2_tag.string = '常见问题 / Frequently Asked Questions'
                    inner_section_tag.append(h2_tag)
                    hr_tag = soup.new_tag("hr")
                    inner_section_tag.append(hr_tag)
                    for a in jsondict['overviews']['faq']:
                        if a is not None:
                            article_tag = soup.new_tag("article")
                            article_tag['class'] = 'response'
                            h3_tag = soup.new_tag("h3")
                            h3_tag.string = a['h3']
                            p_tag = soup.new_tag("p")
                            p_tag.string = a['p']
                            article_tag.append(h3_tag)
                            article_tag.append(p_tag)
                            inner_section_tag.append(article_tag)
                    section_tag.append(inner_section_tag)
                    root_tag.append(section_tag)
                jsondict['overview'] = root_tag.prettify()

        if dirty:
            module_store.update_item(descriptor, user.id)

        if 'recommended' in jsondict:
            if (RecommendedCourses.objects.filter(unicodecoursekey=course_key).exists()) and \
                jsondict['recommended'] == 'false':
                RecommendedCourses.objects.filter(unicodecoursekey=course_key).delete()
            elif (not RecommendedCourses.objects.filter(unicodecoursekey=course_key).exists()) and \
                jsondict['recommended'] == 'true':
                RecommendedCourses.objects.create(unicodecoursekey=course_key)

        if 'facets' in jsondict:
            for facet in jsondict['facets']:
                if 'course_facet_' + facet in jsondict:
                    if jsondict['course_facet_' + facet] != jsondict['facets'][facet]:
                        FacetOfCourse.objects.filter(course=course_key, facet=facet).delete()
                        if jsondict['course_facet_' + facet] != '':
                            FacetOfCourse.objects.create(course=course_key, facet=facet,
                                                         group=jsondict['course_facet_' + facet])

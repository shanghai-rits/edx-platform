    ...
    def clean(self):
        ...
        try:
            if backend.name == 'weixin':
                kwargs = {'response': {'openid':self.request._data.get('openid'),'unionid':self.request._data.get('unionid')}}
                user = backend.do_auth(access_token, allow_inactive_user=True, **kwargs)
            else:

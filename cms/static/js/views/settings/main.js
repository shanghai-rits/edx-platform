define(['js/views/validation', 'codemirror', 'underscore', 'jquery', 'jquery.ui', 'js/utils/date_utils',
    ...
    'js/views/learning_info', 'js/views/instructor_info', 'edx-ui-toolkit/js/utils/string-utils', 'edx-ui-toolkit/js/utils/html-utils'],
       function(ValidatingView, CodeMirror, _, $, ui, DateUtils, FileUploadModel,
                FileUploadDialog, LicenseView, LicenseModel, NotificationView,
                timepicker, date, gettext, LearningInfoView, InstructorInfoView, StringUtils, HtmlUtils) {
    ...
    // Model class is CMS.Models.Settings.CourseDetails
        ...
        // would love to move to a general superclass, but event hashes don't inherit in backbone :-(
                   ...
                   'click .remove-staff': 'deleteOverviewStaff',
                   'click .append-staff': 'addOverviewStaff',
                   'click .remove-faq': 'deleteOverviewFaq',
                   'click .append-faq': 'addOverviewFaq'
               },
        ...
        // handle facets separately, because they are not fixed
                   for (facet in this.model.get('facets')) {
                    this.selectorToField["course-facet-" + facet]="course_facet_" + facet
                    }    
        ...
        // Clear any image preview timeouts set in this.updateImagePreview
                   ...
                   if (this.model.get('recommendation_enabled') == 'nonstaff') {
                    this.$('#recommend-nonstaff').removeAttr('hidden');
                }
                else if (this.model.get('recommendation_enabled') == 'true') {
                    this.$('#' + this.fieldToSelectorMap.recommended).removeAttr('disabled');
                    
                    if (this.model.get('recommended') == 'true') {
                        this.$('#' + this.fieldToSelectorMap.recommended).attr('checked', this.model.get('recommended'));
                    }
                    else {
                        this.$('#' + this.fieldToSelectorMap.recommended).removeAttr('checked');
                    }
                }
                else if (this.model.get('recommendation_enabled') == 'limit') {
                    this.$('#recommend-limit').removeAttr('hidden');
                }

                for (facet in this.model.get('facets')) {
                        this.$el.find("#course-facet-" + facet).val(this.model.get('facets')[facet]);
                }
                   ...
                   if (this.model.get('course_about_html_editor')) {
                    this.$('#' + this.fieldToSelectorMap['course_about_html_editor']).attr('checked', true);
                    this.$('#field-course-overview').show();
                    this.$('#editor-selection').hide();
                    this.$('#course-about-template-editor').hide();
                ...
                //    this.codeMirrorize(null, $('#course-overview')[0]);
                    ...
                    this.$('#' + this.fieldToSelectorMap['course_about_html_editor']).removeAttr('checked');
                    this.$('#field-course-overview').hide();
                    this.$('#editor-selection').show();
                    this.$('#course-about-template-editor').show();

                    if (this.model.get('overviews')['include_course_about']) {
                        this.$('#include-course-about').attr('checked', true);
                        this.$('#course-overview-about-div').show();
                        this.$("#course-overview-about").val(this.model.get('overviews')['about']);
                    }
                    else {
                        this.$('#include-course-about').removeAttr('checked');
                        this.$('#course-overview-about-div').hide();
                    }

                    if (this.model.get('overviews')['include_course_prerequisites']) {
                        this.$('#include-course-prerequisites').attr('checked', true);
                        this.$('#course-overview-prerequisites-div').show();
                        this.$("#course-overview-prerequisites").val(this.model.get('overviews')['prerequisites']);
                    }
                    else {
                        this.$('#include-course-prerequisites').removeAttr('checked');
                        this.$('#course-overview-prerequisites-div').hide();
                    }

                    if (this.model.get('overviews')['include_course_staff']) {
                        this.$('#include-course-staff').attr('checked', true);
                        this.$('#course-overview-staff-list').show();
                        this.$('.append-staff-div').show();
                        this.$('#course-overview-staff-list').children().remove();
                        var staffCollection = this.model.get('overviews')['staff'];
                        _.each(staffCollection, function(staff, index) {
                            if (staff != null) {
                                var args = _.clone(staff);
                                args['index'] = index;
                                HtmlUtils.append(this.$("#course-overview-staff-list"), HtmlUtils.template($('#course_overview_staff-tpl').text())(args));
                            }
                        });
                    }
                    else {
                        this.$('#include-course-staff').removeAttr('checked');
                        this.$('#course-overview-staff-list').hide();
                        this.$('.append-staff-div').hide();
                    }

                    if (this.model.get('overviews')['include_course_faq']) {
                        this.$('#include-course-faq').attr('checked', true);
                        this.$('#course-overview-faq-list').show();
                        this.$('.append-faq-div').show();
                        this.$('#course-overview-faq-list').children().remove();
                        var faqCollection = this.model.get('overviews')['faq'];
                        _.each(faqCollection, function(faq, index) {
                            if (faq != null) {
                                var args = _.clone(faq);
                                args['index'] = index;
                                HtmlUtils.append(this.$("#course-overview-faq-list"), HtmlUtils.template($('#course_overview_faq-tpl').text())(args));
                            }
                        });
                    }
                    else {
                        this.$('#include-course-faq').removeAttr('checked');
                        this.$('#course-overview-faq-list').hide();
                        this.$('.append-faq-div').hide();
                    }
                }
                   ...
                   var courseVerticalImageURL = this.model.get('course_vertical_image_asset_path');
                   this.$el.find('#course-vertical-image-url').val(courseVerticalImageURL);
                   this.$el.find('#course-vertical-image').attr('src', courseVerticalImageURL);
               ...
               fieldToSelectorMap: {
                   ...
                   course_about_html_editor: 'course-about-html-editor',
                   ...
                   course_vertical_image_asset_path: 'course-vertical-image-url',
        ...
        * */
               ...
               addOverviewStaff: function(e) {
                e.preventDefault();
                var newOverviews = $.extend(true, {}, this.model.get('overviews'));
                var length = newOverviews['staff'].push({
                    h3: '',
                    img: '',
                    p: '',
                    link: ''
                });
                this.model.set('overviews', newOverviews);
                this.render();
           },

           deleteOverviewStaff: function(e) {
               e.preventDefault();
               var index = e.currentTarget.getAttribute('data-index');
               this.model.get('overviews')['staff'].splice(index, 1, null);
               this.render();
               this.showNotificationBar();
           },

           addOverviewFaq: function(e) {
                e.preventDefault();
                var newOverviews = $.extend(true, {}, this.model.get('overviews'));
                var length = newOverviews['faq'].push({
                    h3: '',
                    p: ''
                });
                this.model.set('overviews', newOverviews);
           ...
           deleteOverviewFaq: function(e) {
               e.preventDefault();
               var index = e.currentTarget.getAttribute('data-index');
               this.model.get('overviews')['faq'].splice(index, 1, null);
               this.render();
               this.showNotificationBar();
           },
               ...
               updateModel: function(event) {
                if (event.currentTarget.id.indexOf("course-facet-") == 0 ) {
                    this.setField(event);
                } else {
                   var value;
                   ...
                   switch (event.currentTarget.id) {
                    case 'course-about-html-editor':
                        if ($(event.currentTarget).is(':checked')) {
                            this.$('#field-course-overview').show();
                            this.$('#editor-selection').hide();
                            this.$('#course-about-template-editor').hide();
                            this.$('#' + this.fieldToSelectorMap['overview']).val(this.model.get('overview'));
                            this.codeMirrorize(null, $('#course-overview')[0]);
                        }
                        this.setField(event);
                        break;

                    case 'include-course-about':
                        if ($(event.currentTarget).is(':checked')) {
                            this.$('#course-overview-about-div').show();
                        }
                        else {
                        ...
                        newOverviews['include_course_about'] = $(event.currentTarget).is(':checked');
                        this.model.set('overviews', newOverviews);
                        break;

                    case 'include-course-prerequisites':
                        if ($(event.currentTarget).is(':checked')) {
                            this.$('#course-overview-prerequisites-div').show();
                        }
                        else {
                        ...
                        newOverviews['include_course_prerequisites'] = $(event.currentTarget).is(':checked');
                        this.model.set('overviews', newOverviews);
                        break;

                    case 'include-course-staff':
                        if ($(event.currentTarget).is(':checked')) {
                            this.$('#course-overview-staff-list').show();
                            this.$('.append-staff-div').show();
                        }
                        ...
                        newOverviews['include_course_staff'] = $(event.currentTarget).is(':checked');
                        this.model.set('overviews', newOverviews);
                        break;

                    case 'include-course-faq':
                        if ($(event.currentTarget).is(':checked')) {
                            this.$('#course-overview-faq-list').show();
                            this.$('.append-faq-div').show();
                        }
                        ...
                        newOverviews['include_course_faq'] = $(event.currentTarget).is(':checked');
                        this.model.set('overviews', newOverviews);
                        break;

                    case 'course-overview-about':
                        newOverviews['about'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-prerequisites':
                        newOverviews['prerequisites'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-staff-h3':
                        newOverviews['staff'][index]['h3'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-staff-img':
                        newOverviews['staff'][index]['img'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-staff-link':
                        newOverviews['staff'][index]['link'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-staff-p':
                        newOverviews['staff'][index]['p'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-faq-h3':
                        newOverviews['faq'][index]['h3'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                    case 'course-overview-faq-p':
                        newOverviews['faq'][index]['p'] = event.currentTarget.value;
                        this.model.set('overviews', newOverviews);
                        break;
                   ...
                   case 'course-image-url':
                       ...
                       break;
                    case 'course-vertical-image-url':
                        this.updateVerticalImageField(event, 'course_vertical_image_name', '#course-vertical-image');
                        break;
        ...
        // Wait for a second to load the video, avoiding egregious AJAX calls.
            ...
            // Fallthrough to handle both radio buttons
               ...
               updateImageField: function(event, image_field, selector) {
                   ...
                   var image_name = url;
                   // if image path is url then we simply use the url
               ...
               updateVerticalImageField: function(event, image_field, selector) {
                this.setField(event);
                var url = $(event.currentTarget).val();
                this.model.set(image_field, url);
                this.updateImagePreview(event.currentTarget, selector);
                },

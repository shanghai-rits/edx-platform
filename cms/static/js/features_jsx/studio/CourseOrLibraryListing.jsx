...
export function CourseOrLibraryListing(props) {
  ...
  {
      if(!confirm(_('Are you sure? Course deleted cannot be recovered.')))e.preventDefault();
  }
        ...
        props.items.map((item, i) =>
          (
            <li key={i} className="course-item" data-course-key={item.course_key}>
              { props.recommendation && props.recommendation.includes(item.course_key) &&
                <span style={{position: "absolute", left: "0px", top: "0px"}}>
                  <img src={thumbLink} width="50" height="50"/>
                </span>
              }
              ...
              <ul className="item-actions course-actions">
                <li className="action action-delete">
                  <a 
                    href={`/delete_course/${item.course_key}`} 
                    className="button delete-button" 
                    onClick={deleteClicked}
                    aria-labelledby={`delete-${idBase}-${i} title-${idBase}-${i}`}
                    id={`delete-${idBase}-${i}`}
                    >{gettext('Delete Course')}</a>
                </li>
                { allowReruns &&

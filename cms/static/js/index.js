define(['domReady', 'jquery', 'underscore', 'js/utils/cancel_on_escape', 'js/views/utils/create_course_utils',
        'js/views/utils/create_library_utils', 'common/js/components/utils/view_utils', 'js/views/utils/create_facet_utils'],
    function (domReady, $, _, CancelOnEscape, CreateCourseUtilsFactory, CreateLibraryUtilsFactory, ViewUtils, CreateFacetUtilsFactory) {
        'use strict';
        var CreateCourseUtils = new CreateCourseUtilsFactory({
            name: '.new-course-name',
            chinese_name:'.new-course-chinese-name',
            org: '.new-course-org',
        ...
        var CreateFacetUtils = new CreateFacetUtilsFactory({
            name: '.new-facet-name',
            comment: '.new-facet-comment',
            save: '.new-facet-save',
            errorWrapper: '.create-facet .wrap-error',
            errorMessage: '#facet_creation_error',
            tipError: '.create-facet  span.tip-error',
            error: '.create-facet .error',
            allowUnicode: '.allow-unicode-facet-id'
        }, {
            shown: 'is-shown',
            showing: 'is-showing',
            hiding: 'is-hiding',
        ...
        var saveNewCourse = function(e) {
            ...
            var chinese_display_name = $newCourseForm.find('.new-course-chinese-name').val();
        ...
        var saveNewFacet = function(e) {
            e.preventDefault();

            if (CreateFacetUtils.hasInvalidRequiredFields()) {
                return;
            }

            var $newFacetForm = $(this).closest('#create-facet-form');
            var display_name = $newFacetForm.find('.new-facet-name').val();
            var comment = $newFacetForm.find('.new-facet-comment').val();
            ...
            analytics.track('Created a Facet', facet_info);
            CreateFacetUtils.create(facet_info, function (errorMessage) {
                $('.create-facet .wrap-error').addClass('is-shown');
                $('#facet_creation_error').html('<p>' + errorMessage + '</p>');
                $('.new-facet-save').addClass('is-disabled').attr('aria-disabled', true);
            });
        };

        var addNewFacet = function(e) {
            e.preventDefault();
            $('.new-facet-button').addClass('is-disabled').attr('aria-disabled', true);
            $('.new-facet-save').addClass('is-disabled').attr('aria-disabled', true);
            var $newFacet = $('.wrapper-create-facet').addClass('is-shown');
            var $cancelButton = $newFacet.find('.new-facet-cancel');
            var $facetName = $('.new-facet-name');
            $facetName.focus().select();
            $('.new-facet-save').on('click', saveNewFacet);
            $cancelButton.bind('click', makeCancelHandler('facet'));
            CancelOnEscape($cancelButton);

            CreateFacetUtils.configureHandlers();
        };
        ...
        var showTab = function(tab) {
            return function(e) {
                ...
                $('.facets-tab').toggleClass('active', tab === 'facets');
        ...
        var onReady = function() {
            ...
            var facetsTabHref = $('#course-index-tabs .facets-tab a').attr('href');
            

            $('.new-course-button').bind('click', addNewCourse);
            $('.new-facet-button').bind('click', addNewFacet);
            ...
            if (facetsTabHref === '#') {
                $('#course-index-tabs .facets-tab').bind('click', showTab('facets'));
            }

...
# Pattern to match a facet key only
FACET_KEY_PATTERN = r'(?P<facet_key_string>[^/+]+)'
...
urlpatterns = oauth2_urlpatterns + [
    ...
    url(fr'^delete_course/{settings.COURSE_KEY_PATTERN}$', contentstore_views.delete_course_handler,
    ...
    url(r'', include('common.djangoapps.facet.urls')),
]
# pdf_xblock module add by zhxt
urlpatterns += (
    url(r'', include('common.djangoapps.pdf_xblock.urls')),
)
...
if settings.FEATURES.get('ENABLE_FACETS'):
    urlpatterns += [
        url(fr'^facet/{FACET_KEY_PATTERN}?$', 
            contentstore_views.facet_handler, 
            name='facet_handler'),
    ]

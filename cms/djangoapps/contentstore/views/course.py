...
from openedx.core.djangoapps.content.course_overviews.models import CourseOverviewExpansion
from openedx.core.djangoapps.credit.api import get_credit_requirements, is_credit_course
from openedx.core.djangoapps.credit.tasks import update_credit_course_requirements
from openedx.core.djangoapps.models.course_details import CourseDetails
from openedx.core.djangoapps.site_configuration import helpers as configuration_helpers
from openedx.core.djangoapps.user_api.views import check_user_is_teacher
...
from .facet_group import FACETS_ENABLED
from common.djangoapps.facet.models import Facet, Group
...
@ensure_csrf_cookie
@require_http_methods(["GET"])
def delete_course_handler(request, course_key_string):
    """
    The restful handler for course deletion.
    GET
        html: return html page with given course deleted
    """
    # Only global staff (PMs) are able to delete courses
    if not GlobalStaff().has_user(request.user):
        raise PermissionDenied()
    course_key = CourseKey.from_string(course_key_string)
    if (RecommendedCourses.objects.filter(unicodecoursekey=str(course_key)).exists()):
        RecommendedCourses.objects.filter(unicodecoursekey=str(course_key)).delete()
    CourseOverviewExpansion.objects.filter(course_overview=str(course_key)).delete()
...
def course_listing(request):
    ...
    """
    if not check_user_is_teacher(request.user):
        return redirect('{LMS_BASE}'.format(
    ...
    for facet in Facet.objects.all():
        facets.append({'displayname': facet.displayname,
                       'number': facet.number,
                       'items': Group.objects.filter(facet=facet.number).values_list('group', flat=True)})
    ...
    return render_to_response('index.html', {
        ...
        'facets_enabled': FACETS_ENABLED and request.user.is_staff,
        'facets': facets,
        'show_new_facet_button': FACETS_ENABLED and request.user.is_staff,
        'recommendation': RecommendedCourses.objects.values_list('unicodecoursekey', flat=True),
        'user': user,
...
def _create_or_rerun_course(request):
    ...
    try:
        ...
        eng_display_name = request.json.get('display_name')
        chinese_display_name = request.json.get('chinese_display_name')

        display_name = f"{eng_display_name} / {chinese_display_name}"
        ...
        if source_course_key:
            ...
            destination_course_key = rerun_course(request.user, source_course_key, org, course, run, eng_display_name, chinese_display_name, fields)
            return JsonResponse({
            ...
            try:
                new_course = create_new_course(request.user, org, course, run, eng_display_name, chinese_display_name, fields)
                return JsonResponse({
...
def create_new_course(user, org, number, run, eng_display_name, chinese_display_name, fields):
    """
    ...
    # Expand CourseOverview
    CourseOverviewExpansion.objects.create(course_overview=str(new_course.id),
                                           chinese_display_name=chinese_display_name,
                                           eng_display_name=eng_display_name)
...
def create_new_course_in_store(store, user, org, number, run, fields):
    ...
    fields.update({
        ...
        'course_about_html_editor': False,
    })
...
def rerun_course(user, source_course_key, org, number, run, eng_display_name, chinese_display_name, fields, background=True):
    """
    ...
    args = [str(source_course_key), str(destination_course_key), user.id, eng_display_name, chinese_display_name, json_fields]
...
def settings_handler(request, course_key_string):  # lint-amnesty, pylint: disable=too-many-statements
    ...
    with modulestore().bulk_operations(course_key):
        ...
        if 'text/html' in request.META.get('HTTP_ACCEPT', '') and request.method == 'GET':
            ...
            facets = []
            for facet in Facet.objects.all():
                if Group.objects.filter(facet=facet.number):
                    facets.append({'displayname': facet.displayname,
                                   'number': facet.number,
                                   'comment': facet.comment,
                                   'items': Group.objects.filter(facet=facet.number).values_list('group', flat=True)})
            ...
            if request.method == 'GET':
                ...
                if not request.user.is_staff:
                    course_details.recommendation_enabled = 'nonstaff'
                return JsonResponse(
                    course_details,

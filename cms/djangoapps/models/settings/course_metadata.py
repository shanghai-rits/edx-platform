...
class CourseMetadata:
    ...
    def fetch_all(cls, descriptor, filter_fields=None):
        ...
        for field in descriptor.fields.values():
            ...
            course_overview_expansion = CourseOverviewExpansion.objects.get(course_overview=str(descriptor.id))
            
            result['chinese_display_name'] = {
                'value': course_overview_expansion.chinese_display_name,
                'display_name': _('Course Chinese Display Name'),
                'help': _('Enter the Chinese name of the course as it should appear in the edX.org course list.'),
                'deprecated': False
            }
            result['display_name'] = {
                'value': course_overview_expansion.eng_display_name,
                'display_name': _('Course English Display Name'),  # pylint: disable=translation-of-non-string
                'help': _('Enter the English name of the course as it should appear in the edX.org course list.'),  # pylint: disable=translation-of-non-string
                'deprecated': False
            }
    ...
    def validate_and_update_from_json(cls, descriptor, jsondict, user, filter_tabs=True):
        ...
        course_overview_expanseion = CourseOverviewExpansion.objects.get(course_overview=descriptor.id)
        if 'chinese_display_name' in jsondict and jsondict['chinese_display_name'] != course_overview_expanseion.chinese_display_name:
            course_overview_expanseion.chinese_display_name = jsondict['chinese_display_name']['value']
            course_overview_expanseion.save()

        course_overview_expanseion.eng_display_name = jsondict['display_name']['value']
        course_overview_expanseion.save()
        key_values['display_name'] = u"{} / {}".format(jsondict['display_name']['value'],
                                                       jsondict['chinese_display_name']['value'])

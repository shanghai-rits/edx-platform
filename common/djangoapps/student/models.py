...
from common.djangoapps.track import contexts, segment, rudderstack
    ...
    def emit_event(self, event_name, enterprise_uuid=None):
        ...
        try:
            ...
            with tracker.get_tracker().context(event_name, context):
                ...
                rudderstack.track(self.user_id, event_name, segment_properties, traits=segment_traits)
...
def log_successful_logout(sender, request, user, **kwargs):  # lint-amnesty, pylint: disable=unused-argument
    ...
    if hasattr(request, 'user'):
        ...
        if request.user.id:
            ...
            rudderstack.track(request.user.id, 'edx.bi.user.account.logout')
...
class UserExtension(models.Model):
    ...
    user = models.OneToOneField(User, unique=True, db_index=True, related_name='extension', on_delete=models.CASCADE)
    displayname = models.CharField(blank=True, max_length=255)

    @classmethod
    def get_displayname(cls, user, default='default'):
        try:
            user_extension = cls.objects.get(user=user)
            return user_extension.displayname
        except cls.DoesNotExist:
            cls.objects.create(user=user, displayname=default)
            return cls.get_displayname(user, default)
    
    @classmethod
    def check_username_exists(cls, displayname):
        from openedx.core.djangoapps.user_api import accounts as accounts_settings

        if cls.objects.filter(displayname=displayname).exists():
            return _(accounts_settings.USER_DISPLAYNAME_CONFLICT_MSG).format(Nickname=displayname)
        else:
            return ''

...
def index(request, extra_context=None, user=AnonymousUser()):
    ...
    # courses facet list
    if Facet.objects.filter(displayname=settings.HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME).count() < 1:
        Facet.objects.create(displayname=settings.HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME)
    number = Facet.objects.get(displayname=settings.HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME).number

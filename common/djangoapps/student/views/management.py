...
def index(request, extra_context=None, user=AnonymousUser()):
    ...
    def sort_by_enrollment(courses):
        """
        Sort a list of courses by their number of enrollment.
        """
        key = lambda course: CourseEnrollment.objects.num_enrolled_in_exclude_admins(course.id)
    ...
    ## Remove the courses that are archived or did not start yet
    courses = [course for course in courses if (course.has_started() == True and course.has_ended() == False)]

    recommended_courses = [course for course in courses if (RecommendedCourses.objects.filter(unicodecoursekey=str(course.id)).exists())]
    # if len(recommended_courses) < 4:
    #     not_recommended_courses = [course for course in courses if course not in recommended_courses]
    #     recommended_courses.extend(not_recommended_courses)
    popular_courses = sort_by_enrollment(courses)
    ## Add recommended course to the first
    popular_courses = recommended_courses + [course for course in courses if course not in recommended_courses]

    context = {'courses': popular_courses, 'recommendations': recommended_courses}
    ...
    context['courses_list'] = theming_helpers.get_template_path('recommended_courses_list.html')

    # courses facet list
    default_facet = configuration_helpers.get_value('HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME', settings.HOMEPAGE_SUBJECTS_DEFAULT_FACET_NAME)
    if Facet.objects.filter(displayname=default_facet).count() < 1:
        Facet.objects.create(displayname=default_facet)
    number = Facet.objects.get(displayname=default_facet).number
    context['courses_facet_list'] = Group.objects.filter(facet=number)

...
from common.djangoapps.facet.models import Facet, FacetOfCourse, GroupOfLanguage, Group
...
def courses(request):
    ...
    for facet in Facet.objects.all():
        if FacetOfCourse.objects.filter(facet=facet.number):
            course_discovery_meanings[facet.number] = {'name': _(facet.displayname)}
    if not settings.FEATURES.get('ENABLE_COURSE_DISCOVERY'):
        ...
        ## Add recommended to top of the list
        recommended_courses = [course for course in courses_list if (RecommendedCourses.objects.filter(unicodecoursekey=str(course.id)).exists())]
        courses_list = recommended_courses + [course for course in courses_list if course not in recommended_courses]

        ## Remove the courses that already ended
        courses_list = [course for course in courses_list if course.has_ended() == False]
...
class CourseTabView(EdxFragmentView):
    ...
    def create_page_context(self, request, course=None, tab=None, **kwargs):
        ...
        # course chinese display name
        if request.LANGUAGE_CODE == 'zh-cn':
            course.display_name = CourseOverviewExpansion.objects.get(course_overview=str(course.id)).chinese_display_name
...
def facet_info(request):
    """
    Retrieve all facet info as json.
    """
    return HttpResponse(

...
class WikiAccessMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):  # lint-amnesty, pylint: disable=unused-argument
        ...
        # wiki pages are active user required
        if not request.user.is_active:
            PageLevelMessages.register_warning_message(
                request,
                HTML(_("Before proceeding, please {htmlStart}activate your account{htmlEnd}.".format(
            ...
            return redirect('course_root', str(course_id))

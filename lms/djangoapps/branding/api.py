...
def _footer_copyright():
    ...
    return _(u'\u00A9 {copyright_year} All rights reserved.').format(copyright_year=settings.COPYRIGHT_YEAR)
...
def _footer_logo_img(is_secure):
    ...
    """
    default_local_path = 'nyu_ui/images/NYUSH_Chinese_Logo.jpg'
...
def get_logo_url(is_secure=True):
    ...
    default_local_path = 'nyu_ui/images/logo.png'
...
def get_site_logo_url():
    """
    Return the url for the branded logo image for the specific site to be used.

    Preference of the logo should be,
        Look for site configuration override and return url
        Relative default local image path

    """
    default_local_path = 'nyu_ui/images/opencourse_logo.png'

    logo_url_from_site_config = configuration_helpers.get_value('site_logo_image_url')

    if logo_url_from_site_config:
        return logo_url_from_site_config

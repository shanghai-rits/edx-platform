    ...
    define([
        ...
        function(
            ...
            return FormView.extend({
                el: '#phone_register-form',
                tpl: '#phone_register-tpl',
                ...
                events: {
                    ...
                    'blur input[type="password"]': 'liveValidateConfirmPasswordHandler',
                    'focus input[required], textarea[required], select[required]': 'handleRequiredInputFocus',
                    'click .phoneCode': 'getSMSCode'
                },
            ...
            // These are reset to true on form submission.
                ...
                isPhone: function(event) {
                    ...
                    var isPhone = regex_phone.test($(event.currentTarget).val());
                    if(isPhone){
                        $(".phoneCode").addClass("validPhone");
                        $(".phoneCode").css("background", "#57068c");

                    }else{
                        $(".phoneCode").removeClass("validPhone");
                        $(".phoneCode").css("background", "#57068c82");
                    }
                },
                getSMSCode: function(event) {
                    var el = $(event.currentTarget)[0];
                    var validPhone = $(el).hasClass("validPhone");
                    ...
                    if(validPhone){
                        ...
                        var timer = setInterval(function () {
                            if (time <= 0) {
                                ...
                                $("#phone_register-sms_code").removeClass("codeSent");
                            } else {
                                $(el).addClass("inactive");
                                $(el).css("background", "#57068c82");
                                $("#phone_register-sms_code").addClass("codeSent");
                        ...
                        var data = {
                            phone: $("#phone_register-phone").val(),
                            action: 'register'
                        };
    
                        this.smsModel.set(data);
                        this.smsModel.save();
                    }
                },
                ...
                buildForm: function(data) {
                    ...
                    html.push.apply(html, this.renderFields(exposedOptionalFields, 'exposed-optional-phone_fields'));
                    html.push.apply(html, this.renderFields(optionalFields, 'optional-phone_fields'));
                ...
                render: function(html) {
                    ...
                    if (this.autoSubmit) {
                        $(this.el).hide();
                        $('#phone_register-honor_code, #phone_register-terms_of_service').prop('checked', true);
                        this.submitForm();
                    }

                    $('#phone_register-terms_of_service').prop('checked', true);
                ...
                postRender: function() {
                    ...
                    $('#phone-register .optional-phone_fields').addClass('hidden');
                    $('#phone-register #toggle_optional_phone_fields').change(function() {
                        window.analytics.track('edx.bi.user.register.optional_fields_selected');
                        $('#phone-register .optional-phone_fields').toggleClass('hidden');
                    });
                    ...
                    $('#phone-register .exposed-optional-phone_fields').insertBefore(
                        $('#phone-register .honor_tos_combined').parent()
                    );
                    ...
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#phone-register .checkbox-optional_phone_fields_toggle').insertAfter('#phone-register .required-fields');
                    if (!this.hasOptionalFields) {
                        $('#phone-register .checkbox-optional_phone_fields_toggle').addClass('hidden');
                    }
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#phone-register .checkbox-honor_code').insertAfter('#phone-register .optional-phone_fields');
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#phone-register .checkbox-terms_of_service').insertAfter('#phone-register .optional-phone_fields');
                    ...
                    $('#phone_register-confirm_email').bind('cut copy paste', function(e) {
                        e.preventDefault();
                    });
                ...
                liveValidate: function($el) {
                    ...
                    for (i = 0; i < this.liveValidationFields.length; ++i) {
                        ...
                        data[field] = $('#phone_register-' + field).val();
                    }
                ...
                liveValidateConfirmPasswordHandler: function(event) {
                    var $confirmPassword = $("#phone_register-confirm_password");
                    var $password = $("#phone_register-password");
                
                    var validationDecisions = {validation_decisions: {}},
                        decisions = validationDecisions.validation_decisions,
                        name = $confirmPassword.attr('name'),
                        confirm = $confirmPassword.val() === $password.val(),
                        error = $confirmPassword.data('errormsg-required');
                    decisions[name] = confirm ? '' : error;
                    this.renderLiveValidations($confirmPassword, validationDecisions);
                },

        ...
        function(
            ...
            // These are reset to true on form submission.
                ...
                isPhone: function (event) {
                    ...
                    if(isPhone){
                        $(".text-sms_code").show();
                        $("#register-sms_code").attr("required",true);
                    }else{
                        $(".text-sms_code").hide();
                        $("#register-sms_code").attr("required",false);
                        $("#register-sms_code").val("");
                    }
                },
                getSMSCode: function (event) {
                    ...
                    if(inactive){
                        ...
                        $.ajax({
                            url: '/auth/phone',
                            type: 'POST',
                            data: {phone: $("#register-email").val(),action: 'register'},
                            headers: headers
                        });
                    }
                },
                ...
                postRender: function() {
                    ...
                    $('#register .optional-fields').addClass('hidden');
                    $('#register #toggle_optional_fields').change(function() {
                        window.analytics.track('edx.bi.user.register.optional_fields_selected');
                        $('#register .optional-fields').toggleClass('hidden');
                    });
                    ...
                    $('#register .exposed-optional-fields').insertBefore(
                        $('#register .honor_tos_combined').parent()
                    );
                    ...
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#register .checkbox-optional_fields_toggle').insertAfter('#register .required-fields');
                    if (!this.hasOptionalFields) {
                        $('#register .checkbox-optional_fields_toggle').addClass('hidden');
                    }
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#register .checkbox-honor_code').insertAfter('#register .optional-fields');
                    // xss-lint: disable=javascript-jquery-insert-into-target
                    $('#register .checkbox-terms_of_service').insertAfter('#register .optional-fields');
